...
- 根据 FV-RZ09VD2（`NEWDCERV`）实机验证，修正运行模式与风量值域，增加纳诺怡开关。
- `NEWDCERV` 改用专用实时状态端点，并补充已确认的室外传感器和三项滤网倒计时；未验证的滤网周期设置暂不开放。

## 2.2.5

- 校准 `SMALLERV02` 两个滤网更换倒计时的显示名称：`oaFilClFirTL` 对应回风侧滤网，`reFilExTL` 对应初效滤网；保留既有实体 ID。
...
- 按 FY-25ZM1C 实机报告修正风量值域为 `1=低、3=高`，并仅创建该端点实际返回的室外 PM2.5、室外湿度和室外温度传感器。
- `SMALLERV02` 未提供 `runM`，因此不创建运行模式控件；保留开关、风量和假日模式控制。

## 2.2.2

- 空调当前温度优先使用云端 `inhaleTemperature`（吸入/回风温度），只有云端返回无效值时才回退到可选的外部室温传感器。

## 2.2.1

- 空调配置中的“关联室温传感器”改为可选项；不关联时仍可正常创建和控制空调，仅不显示当前室温。
- 已有关联室温传感器的配置保持原有行为。
